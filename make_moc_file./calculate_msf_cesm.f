! Make the executable calculate_msf_cesm that calculates the MOC 
! To do this on the dutch national supercomputer Snellius
! first load the needed modules by typing:
! module load 2021
...
!-------------------------------------------------------------------
     ...
     &  ULAT, ULONG, DZBC
      ...
      character (len=500) attr_name, attr_valc, dim_name, var_name
     &  ,kmt_file, kmt_atl_file, kmt_indopac_file, in_depths_file ,grid_file
     &  ,u_file, v_file, uet_file, vnt_file, output_file, pbc_file
     ...
     &  ,kmt_file, kmt_atl_file, kmt_indopac_file, in_depths_file ,grid_file
     &  ,u_file, v_file, output_file, u_first_record, v_first_record
     &  ,uet_file, vnt_file, uet_first_record, vnt_first_record
     &  ,verbose, do_msf, do_mht, pbc_file
...
!-------------------------------------------------------------------
     ...
     &  ,uet_file, vnt_file, uet_first_record, vnt_first_record
     &  , output_file, verbose, do_msf, do_mht, pbc_file)
...
!-------------------------------------------------------------------
      ...
      open(1,file=in_depths_file,status='old')
      do k = 1,km
        read(1,*) dz(k)
      enddo
      close(1)
...
!-----------------------------------------------------------------------
      ...
      open(1,file=pbc_file,access='direct',form='unformatted',   
     &          recl=rec_length,status='unknown')
      read(1,rec=1) DZBC
      close(1)
      write(*,*)' read file: ',pbc_file
...
!-------------------------------------------------------------------
  ...
  101   nrec = u_first_record
        ...
        !call check(nf90_open(u_file, NF90_NOWRITE, ncid))
        write(*,*)' Now opening file ',u_file
        call check(nf_open(u_file, NF_NOWRITE, ncid))
        write(*,*)' Now finished opening file ',u_file

        ! Get the varid of the data variable, based on its name.
        !call check(nf90_inq_varid(ncid, "U", varid))
        call check(nf_inq_varid(ncid, "UVEL", varid))

        ! Read the data.
        !call check(nf90_get_var(ncid, varid, U))
        call check(nf_get_var(ncid, varid, U))

        ! Close the file, freeing all resources.
        !call check(nf90_close(ncid))
        call check(nf_close(ncid))

        ! Sometimes undefined values are set to a very high value, set those to zero
        ! This is tricky, so be careful with these lines. For now it works
        do k = 1, km
          where (abs(U(:,:,k)) > 100000.) U(:,:,k) = 0.
          ! in CESM land values are set to -1, this does not make sense, set them to 0
          where (U(:,:,k) .eq. -1 ) U(:,:,k) = 0.
        enddo

       ! this code was for reading from a binary input file
  ...
  102   nrec = v_first_record
        ...
        call check(nf_open(v_file, NF_NOWRITE, ncid))
        write(*,*)' Now finished opening file ',v_file
        ...
        ! Read the data.
        !call check(nf90_get_var(ncid, varid, U))
        call check(nf_get_var(ncid, varid, V))

        ! Sometimes undefined values are set to a very high value, set those to zero
        ! This is tricky, so be careful with these lines. For now it works
        do k = 1, km
          where (abs(V(:,:,k)) > 100000.) V(:,:,k) = 0.
          ! in CESM land values are set to -1, this does not make sense, set them to 0
          where (V(:,:,k) .eq. -1 ) V(:,:,k) = 0.
        enddo
...
!-------------------------------------------------------------------
  ...
  103   nrec = uet_first_record
        ...
        call check(nf_open(uet_file, NF_NOWRITE, ncid))
        write(*,*)' Now finished opening file ',uet_file

        ! Get the varid of the data variable, based on its name.
        !call check(nf90_inq_varid(ncid, "U", varid))
        call check(nf_inq_varid(ncid, "UET", varid))

        ! Read the data.
        !call check(nf90_get_var(ncid, varid, U))
        call check(nf_get_var(ncid, varid, UET))
        ...
        ! This is tricky, so be careful with these lines. For now it works
        do k = 1, km
         where (abs(UET(:,:,k)) > 100000.) UET(:,:,k) = 0.
         ! in CESM land values are set to -1, this does not make sense, set them to 0
         where (UET(:,:,k) .eq. -1 ) UET(:,:,k) = 0.
        enddo
  ...
  104   nrec = vnt_first_record
        ...
        call check(nf_open(vnt_file, NF_NOWRITE, ncid))
        write(*,*)' Now finished opening file ',vnt_file

        ! Get the varid of the data variable, based on its name.
        !call check(nf90_inq_varid(ncid, "U", varid))
        call check(nf_inq_varid(ncid, "VNT", varid))

        ! Read the data.
        !call check(nf90_get_var(ncid, varid, U))
        call check(nf_get_var(ncid, varid, VNT))
        ...
        ! This is tricky, so be careful with these lines. For now it works
        do k = 1, km
         where (abs(VNT(:,:,k)) > 100000.) VNT(:,:,k) = 0.
         ! in CESM land values are set to -1, this does not make sense, set them to 0
         where (VNT(:,:,k) .eq. -1 ) VNT(:,:,k) = 0.
        enddo
...
!-------------------------------------------------------------------
      ...
      ncid = NCCRE(output_file, NCCLOB, ierr)
...
!---------------------------------------------------------------------
      ...
      integer imt,jmt,km,j,k,jj,ny, im, jm
...
!-------------------------------------------------------------------
     ...
     &  , uet_first_record, vnt_first_record
      ...
      character (len=500) :: kmt_file, kmt_atl_file, kmt_indopac_file
     &, in_depths_file, grid_file, u_file, v_file, output_file
     &, uet_file, vnt_file, pbc_file
